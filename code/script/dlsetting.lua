...
luci.sys.call(str)
value = luci.sys.exec("/usr/home/SWupdate.exe -v")
str = string.format("uci set dlsetting.@swver[0].dsuver=%s", value)
luci.sys.call(str)
...
local dsuver = swver:option(DummyValue, "dsuver", translate("DL SW update VER"))
local dsver = swver:option(DummyValue, "dsver", translate("DL Socket VER"))
